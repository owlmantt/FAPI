...
# CRUD операции для заметок с пагинацией и поиском
...
def read_notes(
    search: Optional[str] = Query(None, description="Поиск по заголовку или содержимому"),
    skip: int = Query(0, ge=0, description="Количество записей для пропуска"),
    limit: int = Query(10, ge=1, le=100, description="Максимальное количество записей"),
    current_user: models.User = Depends(security.get_current_user),
    db: Session = Depends(get_db)
):
    # Базовый запрос с фильтром по владельцу
